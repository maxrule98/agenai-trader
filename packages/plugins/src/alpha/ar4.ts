 ...
 */
export function fitAR4(returns: number[]): AR4Coefficients {
 ...
 */
export function predictAR4(
	coefficients: AR4Coefficients,
	recentReturns: [number, number, number, number]
): number {
...
export class AR4Alpha {
	private config: AR4Config;
	private coefficients: AR4Coefficients | null = null;
	private returnHistory: number[] = [];

	constructor(config: AR4Config = AR4ConfigSchema.parse({})) {
	...
	generateSignal(features: Features): AlphaSignal | null {
		...
		const signal: AlphaSignal = {
			...
			explain: `AR(4) predicted return: ${predictedReturn.toFixed(
				6
			)}, R²: ${this.coefficients.rSquared.toFixed(3)}`,
		};
