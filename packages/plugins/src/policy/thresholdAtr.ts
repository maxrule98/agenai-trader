...
export class ThresholdAtrPolicy {
	private config: ThresholdAtrConfig;
	private state: PolicyState = {
		side: null,
		size: 0,
	};

	constructor(config: ThresholdAtrConfig = ThresholdAtrConfigSchema.parse({})) {
		...
		if (this.config.enterThreshold < this.config.exitThreshold) {
			throw new Error(
				"enterThreshold must be >= exitThreshold for proper hysteresis"
			);
		}
	}
	...
	generateAction(signal: AlphaSignal, features: Features): Action | null {
		...
		if (this.state.side === null) {
			...
			if (
				...
				absScore >= this.config.enterThreshold
			) {
				// Close current, open new
				const closeAction = this.buildCloseAction(
					signal,
					features,
					currentPrice
				);
	...
	private buildCloseAction(
		...
		const closeSide: Side = this.state.side === "buy" ? "sell" : "buy";
