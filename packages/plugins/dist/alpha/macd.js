...
export class MACDAlpha {
    ...
    constructor(config = MACDConfigSchema.parse({})) {
     ...
     */
    computeHistogramSignal(histogram) {
        ...
        if (this.state.prevHistogram !== null) {
            ...
            if ((direction > 0 && histogramChange > 0) ||
        ...
        const explain = direction > 0
            ? `MACD histogram positive (${histogram.toFixed(6)}), momentum ${momentumStrength > 0.5 ? "strengthening" : "weakening"}`
            : `MACD histogram negative (${histogram.toFixed(6)}), momentum ${momentumStrength > 0.5 ? "strengthening" : "weakening"}`;
        return { score, conf, explain };
    }
    /**
     * Update internal state
     */
    updateState(macd, signal, histogram) {
