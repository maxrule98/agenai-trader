 ...
 */
export function variance(values: number[], period: number): number {
...
export function macd(
	...
	if (values.length < slowPeriod) return result;

	// Calculate fast and slow EMAs
	const fastEma = ema(values, fastPeriod);
	const slowEma = ema(values, slowPeriod);

	if (isNaN(fastEma) || isNaN(slowEma)) return result;
 ...
 */
export function simpleReturn(current: number, previous: number): number {
	if (previous === 0) return NaN;
